...
function SegmentLabel({
  ...
  const valueEl = showValues && (
    <span className="whitespace-nowrap font-semibold text-[#0f0f0f] text-sm">
      {display}
    </span>
  );
  const pctEl = showPercentage && (
    <span className="rounded-full bg-white px-3 py-1 font-bold text-[#0f0f0f] text-xs shadow-sm">
  ...
  const labelEl = showLabels && (
    <span
      className={cn(
        "block max-w-full truncate font-medium text-xs transition-colors duration-200",
        active ? "text-[#0f0f0f]" : "text-[#8a8a8a]"
      )}
    >
      {stage.label}
    </span>
      ...
      >
        {isHorizontal ? (
          <>
            ...
            <div className="flex h-[16%] w-full items-start justify-center overflow-hidden px-[2px] pt-1">
              {labelEl}
            </div>
          </>
        ) : (
...
export function FunnelChart({
  ...
  const bandColor = gridCfg.bandColor ?? "#f5f5f5";
  const showGridLines = gridEnabled && (gridCfg.lines ?? true);
  const gridLineColor = gridCfg.lineColor ?? "#e5e5e5";
        ...
        <>
          ...
          {data.map((stage, i) => {
            ...
            return (
              ...
              >
                <SegmentLabel
                  active={hoveredIndex === null || hoveredIndex === i}
                  align={labelAlign}
